...
const GetCourses = async () => {
    try {
        const res = await axios.get(process.env.REACT_APP_API + "/courses/page/1", {
            headers: {
...
const getUniqueCodes = async () => {
    try {
        const res = await axios.get(process.env.REACT_APP_API + "/courses/uniquecodes", {
            headers: {
                token: "Bearer " + JSON.parse(localStorage.getItem("user")).accessToken,
            },
        });
...
const getCoursesRating = async () => {
    try {
        const res = await axios.get(process.env.REACT_APP_API + "/courses/rating", {
            headers: {
                token: "Bearer " + JSON.parse(localStorage.getItem("user")).accessToken
            }
        });
...
const getTotalCourses = async () => {
    try {
        const res = await axios.get(process.env.REACT_APP_API + "/courses/count", {
            headers: {
                token: "Bearer " + JSON.parse(localStorage.getItem("user")).accessToken
            }
        });
